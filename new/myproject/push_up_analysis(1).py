...
# 导入依赖模块
import re  # 正则表达式，用于提取JSON字符串
import json  # JSON数据解析，读取动作帧数据
import torch  # PyTorch深度学习框架，核心计算库
import torch.nn as nn  # PyTorch神经网络层模块
import torch.nn.functional as F  # PyTorch神经网络函数（激活、损失等）
import numpy as np  # 数值计算库，处理数组和统计计算
...
# ==============================================================================
# 0. 初始化Gemini大模型（用于生成自然语言分析报告）
# ==============================================================================
# Gemini API密钥（需替换为个人有效密钥，此处为示例密钥）
api_key = os.getenv("GEMINI_API_KEY", "").strip()
# 初始化Gemini客户端
client = genai.Client(api_key=api_key) if api_key else None
# 指定使用的Gemini模型（轻量高速版，适合快速生成短报告）
GEMINI_MODEL = "gemini-2.5-flash"

# ==============================================================================
# 1. 模型路径配置（训练好的动作质量评估模型）
# ==============================================================================
# 预训练模型文件路径（LSTM-TCN融合模型，包含权重和标准动作模板）
MODEL_PATH = r"C:\Users\34926\Desktop\LSTM_TCN_train\push_up_modal.pth"
...
# ==============================================================================
# 加载模型 checkpoint（包含模型权重、标准动作模板等）
# map_location="cpu"：强制在CPU上加载（无需GPU也可运行）
ckpt = torch.load(MODEL_PATH, map_location="cpu")
# 初始化融合质量模型
model = FusionQualityModel()
# 加载模型权重（strict=True：严格匹配权重名称和维度，确保兼容性）
model.load_state_dict(ckpt["model"], strict=True)
# 设置模型为评估模式（禁用dropout、batch_norm等训练阶段的层）
model.eval()
# 加载标准动作模板（高质量俯卧撑的融合特征，用于计算相似度）
good_template = ckpt["good_template"]
# 打印模型加载成功提示
print("✔ 模型加载成功.")
...
def load_action(json_path):
    ...
    # 提取动作帧数据（frames为列表，每个元素是一帧的关键点数据）
    frames = o["frames"]
    clean = []  # 存储处理后的帧特征
    for frame in frames:
        row = []  # 存储一帧的扁平化特征（99维）
        for pt in frame:
            # 关键点数据校验：必须是长度为3的列表（x,y,z坐标），否则填充[0,0,0]
            row.extend(pt if isinstance(pt, list) and len(pt) == 3 else [0, 0, 0])
        clean.append(row)

    # 转换为Tensor格式：[seq_len, 99] → [1, seq_len, 99]（添加batch维度）
    x = torch.tensor(clean, dtype=torch.float32).unsqueeze(0)
    # 序列长度Tensor（模型需要知道实际序列长度，避免padding影响）
...
"""
    ...
    try:
        # 调用Gemini生成内容（模型名称+提示词）
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=prompt
        )
        return response.text  # 返回生成的报告
...
if __name__ == "__main__":
    ...
    try:
        # 步骤1：加载动作数据（转换为模型输入格式）
        x, length = load_action(test_json)

        # 步骤2：计算多维度评分
        print("\n=== 多维度评分计算 (v2.2) ===")
        scores = full_quality_analysis(model, x, length, good_template)

        # 步骤3：调用Gemini生成专业报告并打印
        print("\n=== Gemini 分析报告 (v2.2) ===\n")
        print(gemini_analysis(scores))
